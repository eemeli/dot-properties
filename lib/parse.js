 ...
 * is not included.
 *
 * If `ast` is true, lines are parsed into AST nodes instead.
 *
 * @see https://docs.oracle.com/javase/9/docs/api/java/util/Properties.html#load(java.io.Reader)
 *
 * @param {string} src
 * @param {boolean} ast
 * @returns Array<string | string[]> | Array<Node>
 */
function parseLines(src, ast) {
  ...
  for (let i = 0; i < src.length; ++i) {
    ...
    if (atComment(src, keyStart)) {
      ...
      lines.push(ast ? new Comment(comment) : comment)
      i = commentEnd
      continue
    }
 ...
 *
 * @param {string | (string | string[])[] | Node[]} src
